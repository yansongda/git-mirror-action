...
- 增量同步：每次只推变化的分支与 tag；目标端多余的分支/tag 会被删除（镜像语义）
...
- **可见性跟随源**：每次同步都会校正目标端仓库可见性与源一致（私有→私有）。注意：Gitee 公开仓库的可见性不强制回改（其更新接口对 `private=false` 字符串处理有坑，可能误转私有，且无 `public` 参数可用），历史遗留的“公开被建为私有”仓库可在 Gitee 网页手动改回
