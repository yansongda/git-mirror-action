...
# GitCode 覆盖实现: 建仓传 private=1（而非 true）
assert_contains "$(grep 'gitcode.com/api/v5/user/repos' "$MOCK_LOG_FILE")" "private=1"
unset MOCK_LOG_FILE
...
# 可见性校正请求含 public=true（gitee）与 private=0（gitcode 覆盖实现）
assert_contains "$(cat "$MOCK_LOG_FILE")" "public=true"
assert_contains "$(cat "$MOCK_LOG_FILE")" "private=0"
